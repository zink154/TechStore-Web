  ...
  <div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-left">
          <tr>
            ...
            <th class="px-4 py-3 font-medium text-gray-500">Products</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="cat in categories" :key="cat.id">
            ...
            <td class="px-4 py-3">
              <button @click="openEdit(cat)" class="text-indigo-600 hover:text-indigo-800 text-xs mr-3 cursor-pointer">Edit</button>
              <button @click="deleteCategory(cat.id)" class="text-red-500 hover:text-red-700 text-xs cursor-pointer">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg w-full max-w-sm p-6">
        <h2 class="text-lg font-semibold mb-4">{{ editing ? 'Edit Category' : 'Add Category' }}</h2>
