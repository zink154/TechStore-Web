...
const colors = {
  success: 'bg-green-50 dark:bg-green-900/40 text-green-800 dark:text-green-200 border-green-200 dark:border-green-800',
  error: 'bg-red-50 dark:bg-red-900/40 text-red-800 dark:text-red-200 border-red-200 dark:border-red-800',
  info: 'bg-blue-50 dark:bg-blue-900/40 text-blue-800 dark:text-blue-200 border-blue-200 dark:border-blue-800',
    ...
    <div class="fixed top-4 right-4 z-[100] flex flex-col gap-2 max-w-sm w-full pointer-events-none">
      ...
      >
        <div
          ...
          :key="toast.id"
          :class="[colors[toast.type] || colors.info, 'pointer-events-auto flex items-start gap-3 px-4 py-3 rounded-lg border shadow-lg']"
        >
          <span class="flex-shrink-0 mt-0.5" v-html="icons[toast.type] || icons.info"></span>
          <div class="flex-1">
            <p class="text-sm font-medium">{{ toast.message }}</p>
            <button
              v-if="toast.action"
              @click="handleAction(toast)"
              class="text-sm font-bold underline mt-1 cursor-pointer hover:opacity-80"
            >{{ toast.action.label }}</button>
          </div>
          <button @click="removeToast(toast.id)" class="flex-shrink-0 opacity-60 hover:opacity-100 cursor-pointer">
